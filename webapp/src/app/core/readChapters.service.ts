...
export class ReadChaptersService
{
    ...
    constructor(private db: DatabaseService, private user: UserService)
    {
        this.user.doOnLoginChange(async () =>
        {
            ...
            this.readChapters = this.user.isLoggedIn ? await this.db.users.getReadChapters(this.userId) : []
        })
    }
    ...
    public async markChaptersAsRead(chaptersGUID: string[]): Promise<void>
    {
        ...
        await Promise.all(chaptersGUID.map(chapterId => this.db.users.addReadChapter(this.userId, chapterId)))
    }
    ...
    public async markChaptersAsUnread(chaptersGUID: string[]): Promise<void>
    {
        ...
        await Promise.all(chaptersGUID.map(chapterId => this.db.users.removeReadChapter(this.userId, chapterId)))
    }
}
